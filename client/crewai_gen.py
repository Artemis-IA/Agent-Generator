...
class AgentGenerator:
    def __init__(self):
        ...
        self.model_id = os.getenv("OLLAMA_MODEL", "llama3.2") # Modèle par défaut : LLaMA 3
        self.parameters = {
            "temperature": 0.7,  # Contrôle la créativité
            "max_tokens": 1000,  # Tokens max pour JSON complexe
            "top_p": 1.0,        # Probabilité cumulative
            "frequency_penalty": 0.0,  # Pénalité de répétition
            "presence_penalty": 0.0   # Pénalité de présence
        }
        # Pas besoin de project_id ou API key avec Ollama (local)
        self.model = None
    ...
    def _initialize_model(self):
            if self.model is None:
                ...
                try:
                    ...
                    self.model = ollama  # Utilise directement le client Ollama
